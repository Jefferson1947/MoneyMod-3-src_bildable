...
public class GLSLSandboxShader
{
    ...
    public GLSLSandboxShader(final String name, final InputStream is) throws IOException {
        ...
        if (linked == 0) {
            Main.getLogger().error(GL20.glGetShaderInfoLog(program, GL20.glGetProgrami(program, 35716)));
            return;
        }
    ...
    public int createShader(final String check, final InputStream inputStream, final int shaderType) throws IOException {
        ...
        if (compiled == 0) {
            Main.getLogger().error(GL20.glGetShaderInfoLog(shader, GL20.glGetShaderi(shader, 35716)));
            Main.getLogger().error("Caused by {}", (Object)check);
            return 0;
        }
        return shader;
    }
